...
@click.option("--output", "-o", type=click.Path(), default="./downloads", help="Output directory")
@click.option("--archive", "-a", type=click.Choice(["zip", "tar", "tar.gz", "tar.bz2", "none"]), default="none", help="Archive format")
...
@click.option("--method", "-m", type=click.Choice(["api", "scrape", "browser"]), default="api", 
              help="Access method: api (requires credentials), scrape (public boards), browser (interactive login)")
@click.option("--headless", is_flag=True, help="Run browser in headless mode (with --method=browser)")
@click.pass_context
def download(ctx: click.Context, board_url: str, output: str, archive: str, 
             config_file: Optional[str], resume: bool, method: str, headless: bool) -> None:
    ...
    try:
        asyncio.run(_download_board(board_url, config, archive, resume, method, headless))
...
@click.pass_context
def all_boards(ctx: click.Context, output: str, archive: str, config_file: Optional[str], resume: bool) -> None:
    """Download all boards for the authenticated user."""
    config = _load_config(config_file, output)
    
    if not config.has_api_credentials:
        console.print("[red]Error:[/red] API credentials required. Set PINTEREST_ACCESS_TOKEN in .env file.")
        sys.exit(1)
...
@click.option("--browser-only", is_flag=True, help="Only setup browser login credentials")
@click.pass_context
def setup(ctx: click.Context, force: bool, browser_only: bool) -> None:
    ...
    if not browser_only:
        console.print("[cyan]=== Option 1: Pinterest API (Business Account Required) ===[/cyan]")
        console.print("[dim]Pros: Official API, reliable access to private boards[/dim]")
        console.print("[dim]Cons: Requires business account + app approval[/dim]")
        console.print("  - Convert personal account: https://pinterest.com/settings/convert-to-business")
        console.print("  - Create business account: https://pinterest.com/business/create\n")
        ...
        use_api = click.confirm("Would you like to setup API credentials?", default=False)
    else:
        use_api = False
...
"""
    ...
    console.print("[dim]Pros: No business account needed, handles 2FA, PITA login[/dim]")
    console.print("[dim]Cons: Slower, requires visible browser (can use headless)[/dim]")
    console.print("  Credentials stored securely in .env file and used for auto-login")
    
    use_browser = click.confirm("Would you like to setup browser login credentials?", default=True)
...
def _validate_config(config: Config) -> bool:
    ...
    if not config.is_output_writable():
        console.print(f"[red]Error:[/red] Output directory is not writable: {config.output_dir}")
        return False
...
async def _download_board(board_url: str, config: Config, archive_format: str, resume: bool,
                          method: str = "api", headless: bool = False) -> None:
    ...
    if method == "api":
        if not config.has_api_credentials:
            ...
            console.print("Use --method scrape for public boards or --method browser for interactive login.")
            return
        ...
        async with PinterestClient(config) as client:
            ...
            console.print(f"[green]Found board:[/green] {target_board.name} ({target_board.pin_count} pins)")
        ...
        try:
            ...
            async with PinterestBrowser(config, headless=headless) as browser:
    ...
    async with MediaDownloader(config, board_dir) as downloader:
        ...
        if resume and existing_files:
            console.print(f"[cyan]Found {len(existing_files)} files in output directory[/cyan]")
            # Debug info to help understand pin-what-file mapping
            if len(pins) > 0 and len(existing_files) > len(pins):
                console.print(f"[dim]Note: More files ({len(existing_files)}) than pins ({len(pins)}). Some files may be unrelated.[/dim]")
        ...
        if resume:
            ...
            if skipped_count > 0:
                console.print(f"[dim]Resume: {skipped_count} pins matched existing files[/dim]")
...
async def _download_all_boards(config: Config, archive_format: str, resume: bool) -> None:
    """Download all boards for the authenticated user."""
    global _shutdown_event, _shutdown_requested
    _shutdown_event = asyncio.Event()
    _shutdown_requested = False
    ...
    async with PinterestClient(config) as client:
        ...
        for board in boards:
            ...
            console.print(f"[bold]Processing:[/bold] {board.name} ({board.pin_count} pins)")
...
async def _list_boards(config: Config) -> None:
    ...
    async with PinterestClient(config) as client:
        ...
        for board in boards:
            desc = board.description[:50] + "..." if len(board.description) > 50 else board.description
...
def _print_download_summary(results: list, total_pins: int) -> None:
    ...
    if skipped == 0:
        ...
        if successful + skipped < total_pins:
            ...
            console.print(f"[yellow]  ⚠ {missing} pins from board weren't downloaded (may require login)[/yellow]")
    
    if failed:
        console.print(f"[red]  ✗ {failed} pins failed to download[/red]")
    
    if total_bytes > 0:
        console.print(f"[dim]  Total space used: {Archiver._format_size(total_bytes)}[/dim]")
