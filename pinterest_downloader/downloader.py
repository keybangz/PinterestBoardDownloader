...
class MediaDownloader:
    ...
    async def download_pins(
        ...
        for pin in pins:
            ...
            if expected_path.name in existing and expected_path.exists():
                skipped_results.append(DownloadResult(
                    pin=pin,
                    success=True,
    ...
    async def _download_pin(self, pin: Pin) -> DownloadResult:
        ...
        async with self._semaphore:
            ...
            return DownloadResult(pin, False, error=last_error or "Max retries exceeded")
    ...
    async def _do_download(self, pin: Pin) -> DownloadResult:
        ...
        async with file_lock:
            ...
            try:
                async with session.get(pin.media_url) as response:
                    if response.status == 200:
                        pass
                    elif response.status == 404:
                        return DownloadResult(pin, False, error="Media not found (404)")
                    elif response.status == 403:
                        return DownloadResult(pin, False, error="Access denied (403)")
                    else:
                        raise DownloadError(f"HTTP {response.status}: Failed to download")
                    ...
                    if content_length:
                        ...
                        has_space, free_mb = self.config.check_disk_space(
                            max(self.config.min_free_disk_mb, content_length // (1024 * 1024) + 10)
                        )
                        if not has_space:
                            raise DiskFullError(f"Insufficient disk space (free: {free_mb}MB)")
                    
                    content_hash = hashlib.md5() if self.config.verify_duplicates else None
                    ...
                    try:
                        ...
                        if e.errno == 28:
                            ...
                            raise PermissionDeniedError(f"Permission denied: {temp_path}")
                        raise
                    ...
                    if content_hash:
                        ...
                        if file_hash in self._downloaded_hashes:
                            ...
                            return DownloadResult(
                                pin, True, file_path,
                                "Duplicate (already downloaded)", True
                            )
                    ...
                    try:
                        ...
                        if e.errno == 13:
                            raise PermissionDeniedError(f"Cannot rename to {file_path}")
                        raise
                    
                    logger.debug(f"Downloaded: {file_path.name} ({bytes_downloaded} bytes)")
